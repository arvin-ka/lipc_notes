...
2. تست ارتباط بین دو شبکه (End-to-End Ping)
 ...
 تست آی‌پی تونل: از سرور Site-A آدرس ``10.0.0.2`` را پینگ کنید:
 
``ping -c 4 10.0.0.2``

 تست ارتباط بین دو شبکه محلی (LAN): از یک کلاینت درون شبکه ``192.168.10.0/24`` یک کلاینت یا سیستم در شبکه ``192.168.20.0/24`` را پینگ کنید.

``ping 192.168.20.50``

 3.  بنچمارک سرعت و سنجش پهنای باند (Performance Test)
