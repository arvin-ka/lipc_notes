...
# 🌐 LPIC-1 Study Guide: DNS Resolution, Client Configuration & Network Services (Part 3)

این داکیومنت بخش پایانی از مباحث شبکه LPIC-1 (Topic 109) است که روی نحوه‌ی کارکرد سیستم نام‌گذاری دامنه (DNS)، ابزارهای استعلام و پرس‌وجوی DNS (DNS Query Tools)، پیکربندی سمت کلاینت و عیب‌یابی لایه کاربرد تمرکز دارد.

---

## 🔍 1. نحوه کارکرد سیستم DNS (Domain Name System)

سیستم DNS مسئول تبدیل نام‌های قابل فهم برای انسان (مثل example.com) به آدرس‌های IP قابل فهم برای ماشین (مثل 93.184.216.34) است.

### 🔹 ساختار درختی و هرم سلسله‌مراتبی DNS
1. ا **Root Domain (.):** در بالاترین سطح قرار دارد و توسط Root Name Serverها مدیریت می‌شود .
2. ا Top-Level Domain (TLD):):** پسوندهایی نظیر .com, .org, .net یا کد کشورها مانند .ir
3. ا Second-Level Domain:n:** نام دامنه ثبت‌شده (مانند google در google.com)
4. ا Subdomain:n:** زیردامنه‌های تعیین‌شده توسط مدیر شبکه (مانند mail یا lpic در lpic.example.com).

---

## 🛠️ 2. ابزارهای پرس‌وجوی DNS (DNS Query & Lookup Tools)

کتاب رسمی LPIC-1 ابزارهای زیر را برای تست و عیب‌یابی رکوردها و پاسخ‌های سرورهای DNS معرفی می‌کند:
...
​ابزار اصلی لینوکس‌کاران برای عیب‌یابی دقیق DNS که خروجی مفصلی ارائه می‌دهد:

```
# استعلام ساده رکورد A
dig example.com

# دریافت فقط خروجی آدرس IP (کوتاه و مفید)
dig example.com +short

# استعلام یک نوع رکورد خاص (مانند MX برای ایمیل یا NS برای سرورهای نام)
dig example.com MX
dig example.com NS

# استعلام دامنه مستقیم از یک سرور DNS خاص (مثلاً 8.8.8.8)
dig @8.8.8.8 example.com
```

### ا nslookup (ابزار قدیمی و تعاملی)3️⃣

​هرچند این ابزار قدیمی (Deprecated) است، اما همچنان در سرفصل‌های آزمون وجود دارد و امکان کار تعاملی (Interactive) را فراهم می‌کند:

``nslookup example.com``

### 📋 3. رکوردهای مهم DNS (DNS Record Types)

​در آزمون LPIC-1 شناسايی رکوردهای کاربردی زیر الزامی است:


| نوع رکورد | شرح و کاربرد |
| :--- |:--- |
A | نگاشت نام دامنه به آدرس IPV4
AAAA | نگاشت دامنه به آدرس IPV6
CNAME | نام مستعار (Canonical Name) یا ارجاع یک دامنه به دامنه دیگر
MX | معرف سرورهای دریافت ایمیل (Mail Exchanger) برای دامنه
NS | معرف سرورهای نام مجاز (Name Server) برای آن دامنه
PTR | رکورد اشاره گر برای Reverse DNS Lookup (تبدیل آیپی به اسم)
TXT | ذخیره متون توصیفی (مورد استفاده در امنیت ایمیل مانند SPF و DKIM)
SOA | رکورد مبدا اقتدار (Start of authority) شامل اطلاعات مدیریتی Zone 

### 📂 4. پیکربندی کلاینت‌های شبکه و عیب‌یابی لایه کاربرد

​🔹 پیکربندی خودکار با DHCP

​در اکثر شبکه‌ها، تنظیمات IP، Subnet Mask، Gateway و DNS به طور خودکار از سرور DHCP دریافت می‌شود.

#### درخواست مجدد آدرس IP از DHCP در لینوکس:

```
# آزاد کردن IP جاری
sudo dhclient -r

# درخواست IP جدید از سرور DHCP
sudo dhclient
```

### 🔹 تست پورت‌ها و سرویس‌ها با nc (Netcat)

​ابزار Netcat (nc) که به "چاقوی سوئیسی لینوکس" معروف است، برای بررسی باز بودن پورت‌ها روی سرور مقصد در لایه Transport کاربرد دارد:

```
# بررسی باز بودن پورت SSH (22) روی سرور مقصد
nc -zv 192.168.1.100 22

# بررسی باز بودن پورت وب (80)
nc -zv example.com 80
```

* ``-z`` : فقط بررسی اتصال بدون ارسال داده (Zero-I/O mode)
* ``-v`` : خروجی با جزییات (verbose)

### 📑 5. چک‌لیست کامل عیب‌یابی شبکه در لینوکس (Troubleshooting Flowchart)

​هنگام قطع شدن شبکه، بر اساس اصول LPIC-1 مراحل عیب‌یابی را به ترتیب لایه‌ها انجام دهید:

​لایه فیزیکی / لینک: بررسی اتصال کابل و وضعیت کارت شبکه (ip link show).

​لایه شبکه (IP local): بررسی اختصاص IP به کارت شبکه (ip addr show).

​لایه شبکه (Local Gateway): ارسال پنگ به Default Gateway (ping 192.168.1.1).

​لایه شبکه (اینترنت): ارسال پنگ به یک IP خارجی مانند 8.8.8.8.

​لایه نام‌گذاری (DNS): ارسال پنگ به نام دامنه (ping google.com) یا تست با dig.

​لایه کاربرد / پورت: تست اتصال به پورت سرویس مقصد با nc یا curl.
