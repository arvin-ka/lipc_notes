...
* ا **FTP (File Transfer Protocol):** یکی از قدیمی‌ترین پروتکل‌های انتقال فایل است. **بزرگ‌ترین مشکل FTP عدم امنیت آن است**؛ زیرا داده‌ها، نام‌های کاربری و کلمات عبور به صورت متن ساده (Plaintext) منتقل می‌شوند و هر فردی در مسیر شبکه می‌تواند آن‌ها را شنود کند.
* ا **FTPS (FTP over SSL/TLS):** همان FTP است که یک لایه رمزنگاری SSL/TLS به آن اضافه شده است.
* ا **SCP (Secure Copy Protocol):** یک پروتکل قدیمی‌تر و بسیار سریع بر پایه **SSH** است که برای انتقال فایل‌ها و پوشه‌ها بین دو سیستم در شبکه استفاده می‌شود.
* ا **SFTP (SSH File Transfer Protocol):** یک پروتکل کاملاً مجزا (و غیرمرتبط با FTP) است که روی بستر **SSH** عمل می‌کند. SFTP علاوه بر انتقال فایل، امکانات کامل مدیریت فایل روی سرور (مانند ایجاد دایرکتوری، تغییر مجوزها، حذف فایل‌ها و پیمایش مسیرها) را فراهم می‌آورد.
