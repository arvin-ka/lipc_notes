...
* ا **Zone File (فایل زون):** یک فایل متنی ساده (Text File) روی سرور لینوکس است که شامل تمام اطلاعات، نام‌ها و آدرس‌های IP مرتبط با یک دامنه مشخص می‌باشد.
* ا **Forward Zone:** ترجمه **نام دامنه به آدرس IP** (مثلاً تبدیل `web.example.com` به `192.168.10.50`).
* ا **Reverse Zone:** ترجمه **آدرس IP به نام دامنه** (مثلاً تبدیل `192.168.10.50` به `web.example.com`).
...
* ا `/etc/bind/named.conf`: فایل اصلی که بقیه فایل‌ها را Include می‌کند.
* ا `/etc/bind/named.conf.options`: تنظیمات عمومی مانند پورت‌ها، دسترسی‌ها و Forwarderها.
* ا `/etc/bind/named.conf.local`: محلی که زون‌های اختصاصی خود را در آن تعریف می‌کنیم.
...
1. رکورد SOA (Start of Authority)
...
### ⚠️ ۱. قانون طلایی Serial Number

هر زمان که هر رکوردی را در فایل زون تغییر می‌دهید یا اضافه می‌کنید، حتماً باید مقدار ``Serial`` را افزایش دهید.

* فرمت پیشنهادی: ``YYYYMMDDnn`` (سال + ماه + روز + شماره ویرایش روز).
* مثال: ``2026091501`` یعنی ویرایش اول در تاریخ ۱۵ سپتامبر ۲۰۲۶.
* چرا؟ اگر Serial تغییر نکند، سرورهای Slave متوجه تغییرات فایل نمی‌شوند و همگام‌سازی انجام نمی‌شود.

### ⚠️ ۲. اهمیت نقطه پایانی (Trailing Dot)

در فایل‌های زون، اگر در انتهای یک نام دامنه نقطه (``.``) نگذارید، BIND به‌صورت خودکار نام دامنه اصلی را به انتهای آن می‌چسباند!

اشتباه رایج:
ا www CNAME web.company.local ❌ (تبدیل می‌شود به web.company.local.company.local.)

* شکل درست:

ا www CNAME web.company.local. ✅

### ⚠️ ۳. اعمال تغییرات بدون ری‌استارت کامل سرویس

به‌جای ری‌استارت کردن سرویس BIND9 (که باعث قطع لحظه‌ای خدمات می‌شود)، پس از تغییر فایل‌های زون از دستور Reload استفاده کنید:

```
sudo systemctl reload bind9
# یا استفاده از ابزار rndc
sudo rndc reload
```

### 7️⃣ ابزارها و روش‌های تست و عیب‌یابی (Zone Verification)

قبل از اینکه سرویس را عملیاتی کنید یا پس از تغییر زون‌ها، حتماً از ابزارهای زیر برای اعتبارسنجی استفاده کنید:

### 🛠️ ۱. دستور ``named-checkconf``

برای بررسی سلامت ساختاری فایل‌های اصلی پیکربندی BIND استفاده می‌شود.

``sudo named-checkconf``

(اگر هیچ خروجی ندهد، یعنی کانفیگ‌ها سالم هستند).

### 🛠️ ۲. دستور ``named-checkzone``

برای بررسی سلامت فایل زون، مقادیر Serial و عدم وجود غلط‌های املایی/سینتکسی استفاده می‌شود.
...
```
zone company.local/IN: loaded serial 2026091501
OK
```

### 🛠️ ۳. ابزار ``dig`` برای تست عملی زون

* تست دریافت رکورد A:

``dig @127.0.0.1 web.company.local +short``

* تست دریافت رکورد MX:

``dig @127.0.0.1 company.local MX``

* تست صحت رکورد PTR (Reverse Lookup):

``dig @127.0.0.1 -x 192.168.10.50``

* تست دریافت اطلاعات SOA زون:

``dig @127.0.0.1 company.local SOA``

### 🎯 جمع‌بندی

در این داکیومنت، تمام جزئیات مربوط به ساخت، نگهداری، انواع رکوردها و اعتبارسنجی فایل‌های Zone در BIND9 را آموختیم. با رعایت دقیق قواعد ساختاری (مانند Serial Number و Trailing Dot) و استفاده مداوم از ابزارهای اعتبارسنجی نظیر ``named-checkzone`` و ``dig``، می‌توانید سرویس DNS پایدار و بدون خطایی در لینوکس پیاده‌سازی و مدیریت کنید.
