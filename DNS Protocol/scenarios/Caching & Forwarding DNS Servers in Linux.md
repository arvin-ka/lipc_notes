...
* ا **Forwarding خالص:** فقط درخواست‌ها را به سرور دیگر می‌فرستد.
* ا **Caching Forwarder (ترکیبی):** درخواست را به Upstream می‌فرستد، پاسخ را از آن می‌گیرد، نتیجه را کش می‌کند و به کلاینت تحویل می‌دهد. (این رایج‌ترین و پرکاربردترین حالت در شبکه‌های سازمانی است).
